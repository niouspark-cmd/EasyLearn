...
const PhonicGridItem: React.FC<{ grapheme: string, dark?: boolean }> = ({ grapheme, dark }) => (
  ...
  >
    {dark ? <Volume2 size={32} /> : <span className="text-3xl font-black">{grapheme}</span>}
  </button>
);
...
const WordGridItem: React.FC<{ word: string }> = ({ word }) => (
  <button 
    ...
    className="aspect-[1.2/1] flex flex-col items-center justify-center gap-4 bg-white border-2 border-slate-100 rounded-[3rem] hover:border-[#fb9610] transition-all active:scale-95 hover:shadow-xl shadow-sm group relative"
  >
    <div className="flex gap-1">
      {word.split('').map((char, i) => (
        <span key={i} className="text-4xl font-black text-[#022d62] group-hover:text-[#fb9610] tracking-tighter">{char}</span>
      ))}
    </div>
    <div className="w-12 h-12 bg-[#fb961010] text-[#fb9610] rounded-2xl flex items-center justify-center group-hover:bg-[#fb9610] group-hover:text-white transition-all shadow-inner">
...
const StageCard: React.FC<{ id: number, title: string, subtitle: string, onClick: () => void }> = ({ id, title, subtitle, onClick }) => (
  <button 
    onClick={onClick}
    className="w-full p-8 bg-white rounded-[3rem] border-2 border-slate-100 flex items-center justify-between text-left group transition-all hover:border-[#fb9610] hover:shadow-2xl active:scale-95 mb-4"
  >
    <div className="flex items-center gap-6">
      <div className="w-16 h-16 rounded-[1.5rem] bg-[#fb961015] text-[#fb9610] flex items-center justify-center text-3xl font-black italic">
        {id}
      </div>
      <div>
        <h3 className="text-2xl font-black text-[#022d62] leading-tight">{title}</h3>
        <p className="text-slate-400 font-bold text-base tracking-tight">{subtitle}</p>
      </div>
    </div>
    <div className="w-12 h-12 flex items-center justify-center rounded-full bg-slate-50 text-slate-300 group-hover:bg-[#fb9610] group-hover:text-white transition-all">
  ...
  const renderStageContent = () => {
    switch (currentStage) {
      case 1:
        return (
          <div className="animate-fade-in-up">
            <p className="text-slate-500 font-bold mb-8 px-2 text-lg">Tap buttons to hear sounds!</p>
      ...
      case 2:
        return (
          <div className="animate-fade-in-up">
            <p className="text-slate-500 font-bold mb-8 px-2 text-lg">Look and say the sounds.</p>
            <div className="grid grid-cols-2 gap-4">
              {group1.data?.map(item => (
